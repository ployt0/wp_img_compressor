...
Vagrant.configure("2") do |config|
  ...
  config.vm.network :forwarded_port, guest: 22, host: 8213, id: 'ssh'
  ...
  config.vm.provision "Install pip, docker, and imagemagick.", type: "shell", inline: <<-SHELL
apt-get update
apt-get install -y docker.io python3-pip imagemagick
SHELL
